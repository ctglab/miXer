...
eval "$(jq -r '@sh "KIT=\(.kit) THREADS=\(.threads) MIXER_SUPPORT_DIR=\(.mixer_support_dir) PREPARED_DATASET_DIR=\(.prepared_dataset_dir) MIXER_RESOURCES_CONTAINER=\(.mixer_resources_container) MIXER_SUPPORT_CONTAINER=\(.mixer_support_container) PREPARED_DATASET_CONTAINER=\(.prepared_dataset_container) PAR=\(.par) REF=\(.ref) REF37=\(.ref37) OUTDIR_HMM=\(.outdir_hmm) OUTDIR_VCF=\(.outdir_vcf) OUTDIR_HMM_HOST=\(.outdir_hmm_host) OUTDIR_VCF_HOST=\(.outdir_vcf_host)"' "$CONFIG_PATH")"
...
docker run -it \
               ...
               mixer_docker:latest /bin/bash -c "mkdir -p /app/$OUTDIR_HMM /app/$OUTDIR_VCF && source /app/entrypoints/mixer_hmm_inference.sh"

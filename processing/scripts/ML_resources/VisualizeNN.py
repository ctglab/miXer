...
class Layer():
    ...
    def __line_between_two_neurons(self, neuron1, neuron2, weight=0.4, textoverlaphandler=None):
        ...
        if abs_weight > 0.5: 
            linewidth = 3*abs_weight
        elif abs_weight > 0.8: 
            linewidth =  6*abs_weight
        else:
            linewidth = abs_weight
            
        #check if linewidth exceeds a threshold, to improve readability
    ...
    def draw(self, layerType=0, weights=None, textoverlaphandler=None, training_labels = None):
        j=0 # index for neurons in this layer
        for neuron in self.neurons:            
            i=0 # index for neurons in previous layer
            if layerType == 0:
                neuron.draw(self.neuron_radius, id = training_labels[j])
            else:
...
class NeuralNetwork():
    def __init__(self, number_of_neurons_in_widest_layer, savepath, dataset, accuracy, reconstr_error, training_labels):
        self.number_of_neurons_in_widest_layer = number_of_neurons_in_widest_layer
        self.layers = []
        self.layertype = 0
        ...
        self.reconstr_error = reconstr_error
    ...
    def draw(self, weights_list=None):
        ...
        for i in range( len(self.layers) ):
            layer = self.layers[i]                                
            if i == 0:
                layer.draw( layerType=0, training_labels = self.training_labels)
        ...
        pyplot.axis('off')
        if self.accuracy != 0:
            pyplot.title(self.dataset + ' MLP architecture\nAccuracy on holdout set: {:.2f}%'.format(self.accuracy*100), fontsize=15 )
            figureName="Best_NN_{}".format( self.dataset) + '.png'
        elif self.reconstr_error != 0:
            pyplot.title(self.dataset + ' AE architecture\nReconstr_error (holdout set): {:.2f}%'.format(round(self.reconstr_error,2)), fontsize=15 )
            figureName="Best_NN_{}".format( self.dataset) + '.png'
        pyplot.savefig(os.path.join(self.savepath,figureName), dpi=300, bbox_inches="tight")
        pyplot.close()
...
class DrawNN():
    ...
    def __init__( self, neural_network, weights_list=None, savepath = os.getcwd(), dataset = None, accuracy = 0, reconstr_error = 0, training_labels = None):
        ...
        if training_labels == None:
            self.training_labels = range(neural_network[0])
        else:
            self.training_labels = training_labels
    ...
    def draw( self ):
        ...
        network = NeuralNetwork( widest_layer, self.savepath, self.dataset, self.accuracy, self.reconstr_error, self.training_labels)            
